...
class DataParser {
    ...
    parseLettersFrom(dataPath) {
        ...
        for (let i = 0; i < files.length; i++) {
            ...
            console.log(
                `[Parsing letter ... ${Math.floor((i/files.length) * 100)}% (Skipped ${skippedFiles}/${i} files)`
            );
    ...
    parsePersonsFrom(dataPath) {
        ...
        for (let i = 0; i < files.length; i++) {
            ...
            if (i % 100 === 0) {
                console.log(
                    `[Parsing person information ... ${Math.floor((i/files.length) * 100)}% (Skipped ${skippedFiles}/${i} files)`
                );
            }
